...
<div
  class="flex flex-col h-full {isExpand
    ? 'py-4'
    : 'pt-2 pb-4'} text-gray-500 dark:text-gray-400"
>
  <a
  ...
  >
    ...
    {#if isExpand}<span class="text-2xl font-semibold mb-1">Backoffice</span
      >{/if}
  </a>
  ...
  >
    ...
    {#if $isProfileMenuOpen}
      <div
        class="absolute z-30 {isExpand
          ? 'right-0 bottom-full space-y-2'
          : ' left-16'}"
      >
        <ul
